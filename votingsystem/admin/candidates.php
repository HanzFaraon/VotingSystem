...
<body>
	...
	<ul class="nav nav-pills nav-fill">
		...
		<li class="nav-item">
			<a class="nav-link active" aria-current="page" href="candidates.php">Candidates</a>
		</li>
		<li class="nav-item">
	...
	<div class="h1 text-primary border-bottom border-primary">CANDIDATES</div>
		<div class="container-fluid">	
	...
	<?php
		...
		$sql = "SELECT id, firstname, lastname, position FROM candidates";
		$result = $connect->query($sql);
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
		echo "<tr><td>" . $row["firstname"]. "</td><td>" . $row["lastname"] . "</td><td>". $row["position"]. "</td></td>";
	}
		echo "</table>";} 
			else { echo "0 results"; }
		$connect->close();
